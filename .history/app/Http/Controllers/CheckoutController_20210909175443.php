...

{
 ...
 /**
    ...
    {   
        ...
        Mail::send('emails.confirmationPaiement', [ 'client'=>$client],
        function($message){   	
        $user=Auth::user();   
        $message->from('fastdem@info.com', 'Fastdem paiement');
        $message->to($user->email)->subject('confirmation de payement de votre devis en ligne');
       
        dd('envoyer');
     });

     $user=Auth::user();   
     Mail::to($user->email)->send(new TestMail())->subject('confirmation de payement de votre devis en ligne');
